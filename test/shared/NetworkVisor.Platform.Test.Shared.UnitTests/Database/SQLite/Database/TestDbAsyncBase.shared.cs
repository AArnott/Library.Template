...
// // ***********************************************************************
// <copyright file="TestDbAsyncBase.shared.cs" company="Network Visor">
...
{
    public abstract class TestDbAsyncBase<T> : CoreSQLiteAsyncConnection, IDisposable
    {
        private bool disposedValue;

        protected TestDbAsyncBase(ICoreFileSystem fileSystem, CoreSQLiteConnectionString connectionString, ICoreQueryLogger? queryLogger = null)
            : base(fileSystem, connectionString, queryLogger ?? new CoreQueryLogger(fileSystem.GlobalLogger))
        {
        }

        protected TestDbAsyncBase(string databaseName, ICoreFileSystem fileSystem, bool storeDateTimeAsTicks = true, object? key = null, ICoreQueryLogger? queryLogger = null)
        ...
        protected TestDbAsyncBase(ICoreFileSystem fileSystem, bool storeDateTimeAsTicks = true, object? key = null, ICoreQueryLogger? queryLogger = null)
        ...
        protected TestDbAsyncBase(ICoreFileSystem fileSystem, string path, bool storeDateTimeAsTicks = true, object? key = null, bool wal = true, ICoreQueryLogger? queryLogger = null)
            : this(fileSystem, new CoreSQLiteConnectionString(path, storeDateTimeAsTicks, key: key), queryLogger: queryLogger)
        {
        }

        protected TestDbAsyncBase(ICoreFileSystem fileSystem, string databasePath, CoreSQLiteOpenFlags openFlags, bool storeDateTimeAsTicks = true, ICoreQueryLogger? queryLogger = null)
