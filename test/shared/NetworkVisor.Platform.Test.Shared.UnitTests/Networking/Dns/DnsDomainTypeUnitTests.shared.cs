...
// // ***********************************************************************
// <copyright file="DnsDomainTypeUnitTests.shared.cs" company="Network Visor">
...
{
    /// <summary>
    /// Class DnsDomainTypeUnitTests.
    /// </summary>
    [PlatformTrait(typeof(DnsDomainTypeUnitTests))]

    public class DnsDomainTypeUnitTests : CoreTestCaseBase
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="DnsDomainTypeUnitTests"/> class.
        /// </summary>
        /// <param name="testClassFixture">The fixture for the test class, providing shared context for all tests in the class.</param>
        public DnsDomainTypeUnitTests(CoreTestClassFixture testClassFixture)
        ...
        [InlineData("_sack._tcp.", CoreDomainNameType.ServiceName | CoreDomainNameType.NetworkAgentServicePrefix)]
        [InlineData("_sack._tcp.networkvisor.local.", CoreDomainNameType.NetworkAgentServiceName)]
        [InlineData("C6FC5032F30447DCA15BC0E93A1E0967._sack._tcp.", CoreDomainNameType.NetworkAgentServiceInstancePrefix)]
        [InlineData("C6FC5032F30447DCA15BC0E93A1E0967._sack._tcp.networkvisor.local.", CoreDomainNameType.NetworkAgentServiceInstance)]

        [InlineData("_nvtest._udp.", CoreDomainNameType.ServiceName | CoreDomainNameType.NetworkTestServicePrefix)]
        [InlineData("_nvtest._udp.local.", CoreDomainNameType.NetworkTestServiceName)]
        [InlineData("B90556EECFC641A5A55C1AE35F551ABD._nvtest._udp.", CoreDomainNameType.NetworkTestServiceInstancePrefix)]
        [InlineData("B90556EECFC641A5A55C1AE35F551ABD._nvtest._udp.local.", CoreDomainNameType.NetworkTestServiceInstance)]

        public void DnsDomainType_ParseServiceDomain(string? serviceDomainNameString, CoreDomainNameType serviceDomainTypeTest)
        {
            CoreDomainNameType serviceDomainType = CoreDnsExtensions.ToDomainNameType(serviceDomainNameString!);
            this.TestOutputHelper.WriteLine($"{serviceDomainNameString}: {serviceDomainType}");
            serviceDomainType.Should().Be(serviceDomainTypeTest);

            if (serviceDomainTypeTest.HasFlag(CoreDomainNameType.ServiceInstance))
            {
                serviceDomainNameString.IsValidServiceInstance().Should().BeTrue();
            }

            if (serviceDomainTypeTest.HasFlag(CoreDomainNameType.ServiceName))
            {
                serviceDomainNameString.IsValidServiceName().Should().BeTrue();
            }

            if (serviceDomainTypeTest.HasFlag(CoreDomainNameType.NetworkAgentServicePrefix))
            {
                serviceDomainNameString.IsNetworkAgentService().Should().BeTrue();
            }

            if (serviceDomainTypeTest.HasFlag(CoreDomainNameType.NetworkTestServicePrefix))
            {
                serviceDomainNameString.IsNetworkTestService().Should().BeTrue();
            }
        }
    }
}
