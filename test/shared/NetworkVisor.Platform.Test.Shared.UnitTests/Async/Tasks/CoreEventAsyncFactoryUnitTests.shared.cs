...
// ***********************************************************************
// <copyright file="CoreEventAsyncFactoryUnitTests.shared.cs" company="Network Visor">
...
{
    /// <summary>
    /// Class CoreEventAsyncFactoryUnitTests.
    /// Implements the <see cref="CoreTestClassBase" />.
    /// </summary>
    /// <seealso cref="CoreTestClassBase" />
    [PlatformTrait(typeof(CoreEventAsyncFactoryUnitTests))]

    public class CoreEventAsyncFactoryUnitTests : CoreTestCaseBase
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="CoreEventAsyncFactoryUnitTests"/> class.
        /// </summary>
        /// <param name="testClassFixture">The fixture for the test class, providing shared context for all tests in the class.</param>
        public CoreEventAsyncFactoryUnitTests(CoreTestClassFixture testClassFixture)
        ...
        public void CompilationTest_RareScenarios()
        {
            ...
            Assert.Same(Typeof<Task<Tuple<int, int, int>>>(), Typeof(
                EventAsyncFactory.FromAnyEvent<Func<int, int, int, int>, Tuple<int, int, int>>(
                    h => (a, b, c) =>
                {
                    h(Tuple.Create(a, b, c));
                    return 13;
                },
                    h => target.CustomReturnEvent += h,
                    h => target.CustomReturnEvent -= h)));
        }
        ...
        {
            var target = new EventfulType();
            Task task = EventAsyncFactory.FromActionEvent(h => target.ActionEvent += h, h => target.ActionEvent -= h);
        ...
        {
            var target = new EventfulType();
            Task<int> task = EventAsyncFactory.FromActionEvent<int>(h => target.ActionIntEvent += h, h => target.ActionIntEvent -= h);
        ...
        {
            var target = new EventfulType();
            Task<Tuple<int, int, int, int>> task = EventAsyncFactory.FromAnyEvent<Action<int, int, int, int>, Tuple<int, int, int, int>>(h => (a, b, c, d) => h(Tuple.Create(a, b, c, d)), h => target.CustomEvent += h, h => target.CustomEvent -= h);
            target.OnCustomEvent(3, 5, 7, 23);
            Tuple<int, int, int, int> result = await task;
        ...
        {
            var target = new EventfulType();
            Task<Tuple<int, int, int>> task = EventAsyncFactory.FromAnyEvent<Func<int, int, int, int>, Tuple<int, int, int>>(
                h => (a, b, c) =>
            {
                h(Tuple.Create(a, b, c));
                return 13;
                ...
                h => target.CustomReturnEvent += h,
                h => target.CustomReturnEvent -= h);
            int? eventReturn = target.OnCustomReturnEvent(3, 7, 23);
            Tuple<int, int, int> result = await task;
            Assert.Equal(3, result.Item1);
        ...
        private static Type Typeof<T>(T? t_ = default)
        {
            return typeof(T);
        }
        ...
        private sealed class EventfulType
        {
            ...
            public void OnNongenericEventHandlerEvent() => this.NongenericEventHandlerEvent?.Invoke(this, EventArgs.Empty);

            public void OnGenericEventHandlerEvent(int e) => this.GenericEventHandlerEvent?.Invoke(this, e);

            public void OnPropertyChangedEventHandlerEvent(PropertyChangedEventArgs e) => this.PropertyChangedEventHandlerEvent?.Invoke(this, e);

            public void OnActionEvent() => this.ActionEvent?.Invoke();

            public void OnActionIntEvent(int obj) => this.ActionIntEvent?.Invoke(obj);

            public void OnActionSenderIntEvent(int arg2) => this.ActionSenderIntEvent?.Invoke(this, arg2);

            public void OnCustomEvent(int arg1, int arg2, int arg3, int arg4) => this.CustomEvent?.Invoke(arg1, arg2, arg3, arg4);

            public int? OnCustomReturnEvent(int arg1, int arg2, int arg3) => this.CustomReturnEvent?.Invoke(arg1, arg2, arg3);
        }
    }
}
